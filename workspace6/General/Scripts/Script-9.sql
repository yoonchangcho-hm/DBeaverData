...
-- 7. 게시글과 좋아요 개수
select p.title, count(pl.likeId) as count
from post_like pl
join post p
on pl.postId = p.postId
group by p.title;
...
-- 8. 좋아요가 있는 게시글만
SELECT DISTINCT p.postId, p.title
from post p
join post_like pl
on p.postId = pl.postId;


-- 9. 게시글 작성자와 좋아요 누른 사람 매칭
select p.title, u1.username AS 작성자, u2.username AS 좋아요누른사람 
from post p
join user u1
ON p.userId = u1.userId 
join post_like pl
on p.postId = pl.postId 
join user u2
on pl.userId = u2.userId;


-- 10. 가장 최근 좋아요
SELECT u.userId, u.username, pl.postId, p.title
from post_like pl
join user u
